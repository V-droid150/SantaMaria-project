...
export default function OpnameClient({ rows }: { rows: OpnameRow[] }) {
  ...
  async function submit() {
    setError(null);
    if (filledCount === 0) return setError("Isi minimal satu jumlah fisik");
    setSaving(true);
    try {
    ...
    return (
      <>
        ...
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-zinc-300 bg-white py-16 text-center text-zinc-400">
          ...
          <p className="text-sm">Belum ada produk fisik untuk diopname.</p>
    ...
    <>
      ...
      {done !== null && (
        <div className="flex items-center gap-2 rounded-xl border border-yellow-400/40 bg-yellow-400/10 px-4 py-3 text-sm text-yellow-700">
          ...
          Opname tersimpan. {done} produk disesuaikan stoknya.
        </div>
      )}
          ...
          <table className="w-full min-w-[560px] text-left text-sm">
            <thead>
              <tr className="border-b border-zinc-200 bg-zinc-50 text-xs uppercase tracking-wide text-zinc-500">
                ...
                <th className="px-5 py-3 font-semibold">Stok Sistem</th>
                <th className="px-5 py-3 font-semibold">Stok Fisik</th>
                <th className="px-5 py-3 font-semibold">Selisih</th>
              </tr>
            </thead>
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-zinc-200 bg-white p-4 sm:flex-row sm:items-center">
        ...
        <div className="text-xs text-zinc-500 sm:text-right">
          {filledCount} diisi · {diffCount} berubah
        </div>
        <button
          onClick={submit}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold tracking-tight">
            <ClipboardCheck className="h-6 w-6 text-yellow-500" /> Stock Opname
          </h1>
          <p className="text-sm text-zinc-500">
            Hitung stok fisik, sistem menyesuaikan otomatis & mencatat selisihnya.
          </p>
        </div>
      </div>
    </div>
